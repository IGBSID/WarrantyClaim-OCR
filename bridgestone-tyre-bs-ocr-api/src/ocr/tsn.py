...
def get_ocr_tsn(request):
    ...
    try:
        request_id, is_insert = insert_or_update_request_in_db(request_data, "tsn")
    ...
    for file in files:
        if file:
            try:
                tsn_ocr_endpoint = os.environ['TSN_OCR_ENDPOINT']

                # get damage results from the model            
                resp = requests.post(tsn_ocr_endpoint, files={'file':(file.filename, file.stream, file.content_type, file.headers)})
            ...
            try:
                tsn_azure_folder_name = os.environ['TSN_AZURE_FOLDER_NAME']
                image_url, status = upload_to_azure(file_path, tsn_azure_folder_name)
