...
def validate(request):
    try:
        ...
        request_data["correct_damage_category_code"] = request.form.get('Correct_Damage_Category_Code')
        request_data["is_damage_category_correct"] = request.form.get('Is_Damage_Category_Correct')
        ...
        if  (request is None or  # no request
            ...
            request_data["is_damage_category_correct"] is None):
            print("some error")
    ...
    try:
        if request_data["is_damage_category_correct"] == "1":
            # print("request_data[is_damage_category_correct]" + request_data["is_damage_category_correct"])
            if request_data["correct_damage_category_code"] is None:
                raise APIRequestIncorrectDataError
    except (APIRequestIncorrectDataError, Exception) as ex:
        print("error msg = " + str(ex))
        print("API request data not correct")
    ...
    try:
        request_id = update_validate_request_in_db(request_data)
